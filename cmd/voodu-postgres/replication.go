...
// runtimeStreamingConfPath is the PGDATA-relative path the wrapper
// writes primary_conninfo to at every boot. We DON'T bake the
// primary FQDN into the static asset because that would make the
// whole asset content dependent on PG_PRIMARY_ORDINAL — every
// promote would need the asset re-rendered (only happens on
// `vd apply`), so post-promote standbys would stream from the
// stale primary until the operator re-applied. With a runtime-
// written file the wrapper picks up the current ordinal from env
// every boot and the asset stays static across promotions.
//
// Lives inside PGDATA so it persists across container recreates
// and the wrapper can rewrite it idempotently. Loaded by
// postgresql.conf via `include_if_exists 'voodu-runtime.conf'`
// (PGDATA-relative); the wrapper wires the include line on first
// boot. Postgres reads include_if_exists AFTER include_dir, so
// the runtime file's primary_conninfo wins over anything the
// asset accidentally sets.
const runtimeStreamingConfRelPath = "voodu-runtime.conf"
...
func renderStreamingConf(scope, name string, primaryOrdinal int, replicationUser, replicationPassword string) string {
	// Inputs intentionally unused: kept in the signature for callers
	// that haven't migrated yet. primary_conninfo MOVED to a wrapper-
	// written runtime file (PGDATA/voodu-runtime.conf) so promotes
	// don't require an asset re-render. See runtimeStreamingConfRelPath.
	_ = scope
	_ = name
	_ = primaryOrdinal
	_ = replicationUser
	_ = replicationPassword
	...
	b.WriteString("#\n")
	b.WriteString("# primary_conninfo lives in PGDATA/voodu-runtime.conf, written by\n")
	b.WriteString("# the entrypoint wrapper at every boot from PG_PRIMARY_ORDINAL +\n")
	b.WriteString("# replication credentials. That keeps the asset static across\n")
	b.WriteString("# failovers — promote no longer requires `vd apply` to update\n")
	b.WriteString("# the asset content.\n")
